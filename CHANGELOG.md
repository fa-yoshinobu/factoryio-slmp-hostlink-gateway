...
### Changed
- Updated PLC communication package references to `PlcComm.Slmp` and `PlcComm.KvHostLink` `2.0.0` (canonical module I/O vocabulary release).
- Kept the SLMP route fixed to the directly connected PLC, with the MultiDrop station number always `0x00`.
...
- Switched PLC communication references to the latest NuGet packages: `PlcComm.Slmp` and `PlcComm.KvHostLink` `1.2.0`.
- Updated test packages: `Microsoft.NET.Test.Sdk` `18.7.0`, `xunit.runner.visualstudio` `3.1.5`, and `coverlet.collector` `10.0.1`.
...
- Removed the GitHub Actions sibling repository checkouts that were only needed for local `PlcComm` source project references.
