...
public partial class MainViewModel : ObservableObject, IAsyncDisposable
{
    ...
    public string ModbusCycleText => FormatCycle("Modbus Cycle", _gatewayService.ModbusRequestCycleMilliseconds);

    public string PlcCycleText => FormatCycle("PLC Cycle", _gatewayService.PlcPollCycleMilliseconds);
    ...
    private void RefreshModbusStatus()
    {
        if (!IsRunning)
        {
            return;
        ...
        var key = _gatewayService.ModbusClientCount > 0
            ? "ModbusTcpConnected"
            : "ModbusTcpListening";
    ...
    private static string FormatCycle(string label, double? milliseconds)
    {
    ...
    private static bool TryParseRegisterInput(MappingEntry entry, string input, out int raw, out string error)
        => RegisterInputParser.TryParse(entry, input, out raw, out error);
